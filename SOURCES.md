...
- **URL:** local (see `data/raw/taskmaster_data.csv`)
...
- Source file is saved verbatim to `data/raw/taskmaster_data.csv`. The pipeline reads it untouched; corrections to the raw values are made deliberately and documented here.
...
- **The primary series break is episode count.** Raw `total_points` is not comparable across series. Always compare on `pct_of_series_points` (share of the series' total) or within-series `series_rank`. This is enforced in `03-prepare` and stated in the codebook and every chart footnote.
